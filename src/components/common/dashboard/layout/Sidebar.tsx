...
interface SidebarProps {
  ...
  onLinkClick: () => void; // Add a callback to close the sidebar
}

const Sidebar: React.FC<SidebarProps> = ({ isSidebarOpen, onLinkClick }) => {
  const links = [
    { to: '/dashboard', icon: <AiOutlineHome className="ml-3" />, label: 'نظرة عامة' },
    { to: '/dashboard/companies', icon: <FaBuilding className="ml-3" />, label: 'الشركات' },
