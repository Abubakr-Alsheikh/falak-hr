      ...
      if (refreshToken) {
        try {
          ...
          axiosInstance.defaults.headers.common[
            "Authorization"
          ] = `Bearer ${newAccessToken}`; // Update axios default header
...
export const refreshAccessTokenFn = async (
  ...
  // Renamed to avoid confusion, not directly used now, but can be kept for explicit refresh calls if needed
  if (!refreshToken) {
  ...
  try {
    ...
    clearTokens();
    throw new Error(
      error.message || "Failed to refresh access token. Please log in again."
    );
  }
};
